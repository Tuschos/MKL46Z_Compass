...
// Đinh nghia các thanh ghi cua MAG3110.
#define x_msb 0x01
#define x_lsb 0x02
#define y_msb 0x03
#define y_lsb 0x04
...
{   
    ...
    // Cau hěnh chân cho giao dien I2C0  cho MAG3110
    PORTE->PCR[24] = PORT_PCR_MUX(5);
    PORTE->PCR[25] = PORT_PCR_MUX(5);

    // Thiet lap baudrate vŕ kích hoat I2C0.
    I2C0->F  = 0x1F; // baudrate
    I2C0->C1 = 0x80; // enable IIC
}


// Hŕm doc 1 byte tu thanh ghi cua MAG3110 qua I2C.
uint8_t MAG3110_read(uint8_t addr)
{
    uint8_t result=0;
	
    // Thiet lap che do master vŕ truyen
...
// Hŕm ghi 1 byte vŕo thanh ghi cua  MAG3110 qua I2C.
...
void MAG_read(uint8_t buffer[], MAG3110_vector* vector)
{
     ...
     * 1trong các bit dó lŕ 1, có nghia lŕ có du lieu moi,
     * bit 3: 1 - du lieu moi
     * bit 2: 1 - .x
     * bit 1: 1 -  y
     * bit 0: 1 -  z
    ...
    // x lŕ uint16_t, vŕ buffer[] lŕ uint8_t,
      vector->x  = buffer[0] << 8;
      vector->x |= buffer[1];
